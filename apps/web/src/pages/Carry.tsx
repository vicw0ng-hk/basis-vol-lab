...
import { useArtifact, type Carry } from '../lib/api';
...
export default function CarryPage() {
  ...
  const axis = isDark ? 'oklch(0.7 0.02 240)' : 'oklch(0.45 0.02 250)';
  const bg = isDark ? 'oklch(0.2 0.015 250)' : 'white';
  const fg = isDark ? 'oklch(0.96 0.005 240)' : 'oklch(0.18 0.02 250)';

  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">Carry</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Annualized perp funding versus dated-futures basis. Funding history is
          the trailing 100-row window from Binance.
        </p>
      </header>
